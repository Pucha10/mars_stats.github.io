...
async function saveNewGame() {
    ...
    const dSum = document.getElementById("new-sum-dawid")
        ? parseInt(document.getElementById("new-sum-dawid").innerText)
        : 0;
    ...
    if (kSum > dSum) {
        ...
        const choice = confirm(
            "Mamy remis punktowy! Kliknij OK, jeśli wygrała KASIA. Kliknij ANULUJ, jeśli wygrał DAWID.",
        );
    ...
    const gameData = {
        Kasia_corporation: document.querySelector(
            '.kasia-in[data-field="Kasia_corporation"]',
        ).value,
        Kasia_wt:
            parseInt(document.querySelector('[data-field="Kasia_wt"]').value) ||
            0,
        Kasia_awards:
            parseInt(
                document.querySelector('[data-field="Kasia_awards"]').value,
            ) || 0,
        Kasia_titles:
            parseInt(
                document.querySelector('[data-field="Kasia_titles"]').value,
            ) || 0,
        Kasia_board_score:
            parseInt(
                document.querySelector('[data-field="Kasia_board_score"]')
                    .value,
            ) || 0,
        Kasia_cards_score:
            parseInt(
                document.querySelector('[data-field="Kasia_cards_score"]')
                    .value,
            ) || 0,
        Kasia_total_score: kSum,

        Dawid_corporation: document.querySelector(
            '.dawid-in[data-field="Dawid_corporation"]',
        ).value,
        Dawid_wt:
            parseInt(document.querySelector('[data-field="Dawid_wt"]').value) ||
            0,
        Dawid_awards:
            parseInt(
                document.querySelector('[data-field="Dawid_awards"]').value,
            ) || 0,
        Dawid_titles:
            parseInt(
                document.querySelector('[data-field="Dawid_titles"]').value,
            ) || 0,
        Dawid_board_score:
            parseInt(
                document.querySelector('[data-field="Dawid_board_score"]')
                    .value,
            ) || 0,
        Dawid_cards_score:
            parseInt(
                document.querySelector('[data-field="Dawid_cards_score"]')
                    .value,
            ) || 0,
        Dawid_total_score: dSum,

        game_number: parseInt(
            document.getElementById("new_game_number").innerText,
        ),
        winner: finalWinner,
        generations:
            parseInt(document.getElementById("new-generations").value) || 0,
...
async function handleDeleteGame(id) {
    if (
        confirm(
            `Czy na pewno chcesz usunąć grę nr ${id}? Tej operacji nie można cofnąć.`,
        )
    ) {
...
async function handleEditGame(id) {
    ...
    const allEditInputs = [
        ...kasiaRow.querySelectorAll(".edit-input"),
        ...dawidRow.querySelectorAll(".edit-input"),
    ];
...
function calculateEditTotals() {
    let kSum = 0;
    document
        .querySelectorAll(".kasia-edit")
        .forEach((i) => (kSum += parseInt(i.value) || 0));
    let dSum = 0;
    document
        .querySelectorAll(".dawid-edit")
        .forEach((i) => (dSum += parseInt(i.value) || 0));
...
async function saveEdit(id) {
    ...
    if (kSum > dSum) finalWinner = "Kasia";
        ...
        finalWinner = confirm("Remis! OK = Kasia, Anuluj = Dawid")
            ? "Kasia"
            : "Dawid";
    ...
    const updatedData = {
        Kasia_wt:
            parseInt(
                document.querySelector('.kasia-edit[data-field="Kasia_wt"]')
                    .value,
            ) || 0,
        Kasia_awards:
            parseInt(
                document.querySelector('.kasia-edit[data-field="Kasia_awards"]')
                    .value,
            ) || 0,
        Kasia_titles:
            parseInt(
                document.querySelector('.kasia-edit[data-field="Kasia_titles"]')
                    .value,
            ) || 0,
        Kasia_board_score:
            parseInt(
                document.querySelector(
                    '.kasia-edit[data-field="Kasia_board_score"]',
                ).value,
            ) || 0,
        Kasia_cards_score:
            parseInt(
                document.querySelector(
                    '.kasia-edit[data-field="Kasia_cards_score"]',
                ).value,
            ) || 0,
        Kasia_total_score: kSum,
        ...
        Dawid_wt:
            parseInt(
                document.querySelector('.dawid-edit[data-field="Dawid_wt"]')
                    .value,
            ) || 0,
        Dawid_awards:
            parseInt(
                document.querySelector('.dawid-edit[data-field="Dawid_awards"]')
                    .value,
            ) || 0,
        Dawid_titles:
            parseInt(
                document.querySelector('.dawid-edit[data-field="Dawid_titles"]')
                    .value,
            ) || 0,
        Dawid_board_score:
            parseInt(
                document.querySelector(
                    '.dawid-edit[data-field="Dawid_board_score"]',
                ).value,
            ) || 0,
        Dawid_cards_score:
            parseInt(
                document.querySelector(
                    '.dawid-edit[data-field="Dawid_cards_score"]',
                ).value,
            ) || 0,
        Dawid_total_score: dSum,
        ...
        winner: finalWinner,
        generations:
            parseInt(document.getElementById("edit-generations").value) || 0,
